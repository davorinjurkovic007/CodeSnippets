...
namespace XMLSamples {
  ...
  /// </summary>
  public class LINQElementViewModel {
    public LINQElementViewModel() {
    ...
    public List<XElement> GetAllXDocument() {
      ...
      // Write Query Here
	list = (from prod in doc.Descendants("Product") select prod).ToList();
    ...
    public List<XElement> GetAllXElement() {
      ...
      // Write Query Here
	list = (from prod in elem.Elements("Product") select prod).ToList();
    ...
    public List<XElement> WhereClause() {
      ...
      list = (from prod in elem.Elements("Product")
              where prod.GetAs<string>("Color") == "Silver"
              select prod).ToList();
    ...
    public List<XElement> OrderBy() {
      ...
      // Write Query Here
      list = (from prod in elem.Elements("Product")
              orderby prod.GetAs<string>("Color"),
                      prod.GetAs<decimal>("ListPrice", 0)
              select prod).ToList();    
    ...
    public XElement Join() {
      ...
      XElement newDoc = 
                new XElement("SalesOrderWithProductInfo",
                    ...
                    select new XElement("Order",
                        ...
                        new XElement("ListPrice", order.GetAs<decimal>("ListPrice", 0)),
